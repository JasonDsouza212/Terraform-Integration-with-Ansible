...
resource "null_resource" "configre_server" {
  provisioner "local-exec"{
    working_dir = "/home/jason/Documents/practice/terrafrom_integration_with_ansible"
    command= "ansible-playbook --inventory ${aws_instance.myapp-server.public_ip}, --private-key ${var.private_key_loc} --user ec2-user docker_playbook.yaml"
  }
}